...
class VersionConsistencyTests(unittest.TestCase):
    def test_current_framework_release_metadata_is_v220(self):
        version = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
        catalog = json.loads((ROOT / ".gpt-codex" / "builtins" / "INDEX.json").read_text(encoding="utf-8"))
        record = json.loads((ROOT / "releases" / "records" / "v2.2.0.json").read_text(encoding="utf-8"))
        index = json.loads((ROOT / "releases" / "INDEX.json").read_text(encoding="utf-8"))

        self.assertEqual(version, "2.2.0")
        self.assertEqual(catalog["framework_version"], "2.2.0")
        self.assertEqual(record["version"], "2.2.0")
        self.assertEqual(record["kernel_version"], "2.0.0")
        self.assertEqual(record["schema_version"], 1)
        self.assertEqual(index["latest_recorded_version"], "2.2.0")
    ...
    def test_current_docs_and_changelog_name_v220(self):
        ...
        self.assertIn("Framework v2.2.0", readme)
        self.assertIn("Framework v2.2.0", bootstrap)
        self.assertIn("Framework v2.2.0", agents)
