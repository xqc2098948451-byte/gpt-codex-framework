...
- Pairwise role authority is derived only from `STATE.active_execution_slots + REVIEW_REQUEST`: select exactly one `CODEX_REVIEWER` slot matching review-request context, Work Unit, and instruction ID, then exactly one matching `CODEX_IMPLEMENTER` peer. Each root candidate comes from its selected `slot.worktree`; caller-supplied slot or root values are not authority. Zero or multiple matches, missing roots, unavailable Git facts, or unavailable cleanliness fail closed with `RECONCILIATION_REQUIRED`.
...
- A private helper in `continuity_resume.py` derives both selected slots' canonical roots, implementation/reviewer HEADs, reviewer cleanliness, context, Work Unit, and review-request ID by reusing `_execution_slot_recovery(...)` and `_git_recovery_is_current(...)`. It passes only those derived facts through existing `authoritative_review_context` to `validate_pre_execution_review(...)`; it creates no public entrypoint, schema, registry, or persisted context.
