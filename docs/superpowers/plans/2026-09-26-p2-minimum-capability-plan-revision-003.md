...
`build_repository_handoff(root, selected_repository_id, *, target_work_unit_id, target_work_unit_ref, instruction_locator, expected_state_revision, observed_remote_head_sha=None) -> dict[str, Any]`

The helper may use existing validators and smaller private helpers in `continuity_resume.py`. The four keyword-only inputs `target_work_unit_id`, `target_work_unit_ref`, `instruction_locator`, and `expected_state_revision` are required handoff authority bindings and must remain explicit during authorized implementation. Validate them against the exact governed Instruction Envelope, immutable Work Unit artifact, and current STATE-revision authority. Missing bindings must reconcile; neither the native helper nor the Plugin may infer them from `STATE.active_work_unit`, `continuity.last_verified_result_ref`, a branch name, Plugin local state, or a directory scan heuristic. `STATE.active_work_unit` remains an observed STATE fact; it is not silently replaced or reinterpreted.
...
1. Write focused RED tests for slotless/legacy repository baseline resume with all four handoff bindings omitted (baseline facts only, no Result handoff readiness); repository-level handoff with all four explicit bindings and an exact target Work Unit distinct from a legacy `STATE.active_work_unit`; existing valid slot-aware handoff; and the distinction between the two results.
2. Add RED failure tests for only one, two, or three handoff bindings supplied; target Work Unit ID/ref mismatch; Instruction target Work Unit mismatch; expected STATE revision mismatch; slot authority conflicting with explicit handoff bindings; wrong project/context or repository; Instruction locator/ID mismatch; duplicate Result; Result Work Unit mismatch; Result-to-Instruction correlation mismatch; Git SHA mismatch; missing or ambiguous durable continuation; malformed or ambiguous slot authority; and private-context exclusion. Verify that the Plugin never falls back to `STATE.active_work_unit`, substitutes an unrelated `continuity.last_verified_result_ref`, or scans Result files itself.
...
4. Make `build_plugin_resume(root, selected_repository_id, execution_slot_id=None, *, target_work_unit_id=None, target_work_unit_ref=None, instruction_locator=None, expected_state_revision=None) -> dict[str, Any]` call `load_continuity_resume(...)` first. If all four handoff bindings are `None`, return repository-level baseline facts only and do not claim Result handoff readiness. If any binding is supplied, require all four; partial bindings return `RECONCILIATION_REQUIRED` without inference, supplementation, or scanning. Validate the bindings and pass them explicitly to `build_repository_handoff(...)` for a legitimate slotless STATE. Use slot-aware enrichment only when complete unique valid slot authority exists and agrees with every explicit binding; any conflict returns `RECONCILIATION_REQUIRED`. Neither slot authority nor Instruction bindings silently override the other.
5. Run focused continuity, Plugin, and Result tests. Review the response shape to confirm baseline-only resume does not assert Result handoff readiness, no field asserts slot authority on the slotless path, and no `next_authorized_action` is produced.
...
Task 2 Instruction resolution is the sole Plugin-side source of the four handoff bindings. On successful resolution of the exact Instruction Envelope, obtain its exact `instruction_locator`, `instruction_id`, `target_work_unit_id`, immutable `target_work_unit_ref`, and `expected_state_revision`, along with the existing project/context and repository bindings. Validate that these values agree with the resolved Envelope and pass the same four bindings through Task 1 or Task 3 without recomputing alternatives. If any resolved value conflicts with a caller-supplied binding, return `RECONCILIATION_REQUIRED` before Result discovery begins. Add focused tests for these equalities and conflicts. A slotless STATE does not require a slot locator. Where valid slot authority exists, the Instruction's Work Unit ID/ref, Instruction, and STATE revision must agree with that authority; a conflict returns `RECONCILIATION_REQUIRED` without either source overriding the other. An Instruction visibility or publication event is not authorization or proof of Result continuation.
...
- For complete, unique, valid matching slot authority, use existing slot-aware handoff only when slot authority agrees with all four explicit governed bindings.
- For legitimate slotless STATE, pass those bindings to repository-level durable handoff from `continuity_resume.py`.
- For malformed or conflicting slot authority, reconcile; never fall back silently.

Use an interface equivalent to `build_plugin_result_handoff(root, selected_repository_id, *, target_work_unit_id, target_work_unit_ref, instruction_locator, expected_state_revision, execution_slot_id=None) -> dict[str, Any]`. A Result handoff has no baseline-only shortcut: all four bindings are required, and a missing binding returns `RECONCILIATION_REQUIRED`. The adapter passes the validated Task 2 bindings to native `build_repository_handoff(...)` or, under complete valid matching slot authority, uses slot-aware enrichment. It must not ignore explicit bindings, let a slot override an Instruction binding, or let an Instruction silently override a slot conflict.
...
Test both valid paths, missing any of the four required bindings, slot-versus-binding conflicts, and all Task 1 handoff failures. In particular, an unrelated `continuity.last_verified_result_ref` must not be presented as the target Work Unit's Result. Result `PASS` remains distinct from Work Unit `COMPLETE`.
