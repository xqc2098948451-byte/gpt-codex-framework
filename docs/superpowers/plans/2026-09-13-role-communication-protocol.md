...
- A review-visibility push is not production publication. Missing or incomplete synchronization evidence yields `LOCAL_COMPLETE / SYNC_PENDING` when no contradictory remote fact exists. Conflicting or stale synchronization facts yield `RECONCILIATION_REQUIRED`.
...
**Interfaces:** Add result-only `result_message_type` for `REVIEW_RESULT`, `REVIEW_FINDING`, `IMPLEMENTATION_RESULT`, and registered protocol error/result classes. Reject instruction-side values. Add bounded `response_to_instruction_id`, `responder_role`, `return_role`, `review_target_revision`, `finding_ids`, `fix_round`, `remediation_decision_ref`, `protocol_error`, `role_observation`, `artifact_stage`, and `artifact_path`. Uppercase `REPOSITORY`, `BRANCH`, `BASE_SHA`, `HEAD_SHA`, `ARTIFACT_PATH`, `PUSH_STATUS`, and `REMOTE_VERIFICATION` are derived GPT Return/Handoff labels only. Render `INSTRUCTION_TYPE` and `RESULT_MESSAGE_TYPE` from their respective source fields; never render `MESSAGE_TYPE`. Handoff remains derived evidence and cannot authorize execution.

**REUSE EXISTING CANONICAL RESULT FIELDS:** `source_github_repository_full_name`, `current_remote_ref`, `local_head_sha`, `remote_head_sha`, `sync_status`, `push_status`, and `remote_verification` already exist and remain the authoritative machine facts. Reuse an existing canonical Result Envelope field whenever it already represents the required fact.

**ADD GENUINELY NEW RESULT FIELDS ONLY:** `result_message_type`, `response_to_instruction_id`, `responder_role`, `return_role`, `review_target_revision`, `finding_ids`, `fix_round`, `remediation_decision_ref`, `protocol_error`, `role_observation`, `artifact_stage`, and `artifact_path`, subject to the approved design and existing schema conventions. Do not create uppercase duplicate machine properties.
