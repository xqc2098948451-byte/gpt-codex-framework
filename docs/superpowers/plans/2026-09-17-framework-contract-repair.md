...
This amendment supersedes the earlier Task 10, Task 11, publication, lifecycle-checkpoint sequencing, and the preceding historical summary sentence that says Tasks 1–10 mutate only bridge-003 paths. That sentence remains historical description of the old accepted Plan, not authority for the amended lifecycle. Tasks 1–4 remain technically unchanged.
...
Bridge `framework-contract-repair-bridge-003` retains its exact immutable old Design/Plan binding. The amendment neither rewrites, reinterprets, nor expands it. It may continue only for operations already authorized by that old authority; because Prelude and Tasks 1–4 retain their technical semantics, only that overlapping portion may be consumed. After Tasks 1–4 verification, execution must stop. Bridge-003 does not authorize the amended checkpoint-release ordering, Phase-A final verification, `2.7.1` release-candidate preparation under this Plan, publication, or Tasks 5–9.

After the corrected phase-split Design and Plan are accepted and Prelude plus Tasks 1–4 pass, future `framework-contract-repair-bridge-004` is required for Phase-A checkpoint publication. It is separately prepared, independently PRE_EXECUTION reviewed, exact-payload USER_APPROVER approved, and remotely verified before use; it binds the corrected Design/Plan exact SHAs. Its exact scope is limited to Phase-A final verification, `2.7.1` release surfaces, one reviewed candidate-commit/publication lifecycle, and remote verification; it is non-reusable. When bridge-004 becomes remotely authorized, bridge-003 is superseded and terminated for further mutation. This Plan does not create bridge-004.

The executable Phase-A order is: bridge-003 immutable old authority; Prelude; Tasks 1–4; verify Tasks 1–4; STOP bridge-003 execution; bridge-004 preparation/review/approval/remote verification; Phase-A final verification; `2.7.1` release-candidate preparation; independent `POST_EXECUTION` review; exact-SHA USER_APPROVER acceptance; governed publication; remote `2.7.1` verification; terminate bridge-004; STOP/PAUSE FEATURE DEVELOPMENT. The Phase-A candidate must pass Prelude, Tasks 1–4, the full test suite, `validate_framework.py`, `validate_project.py .`, `validate_consumer_projection.py --root .`, `git diff --check`, and authorized changed-path verification before independent review. Publication is permitted only for the exact accepted candidate SHA.

### Phase B — Framework 2.7.2

Former Tasks 5–9 are deferred to `2.7.2` with their technical semantics retained: external approval-evidence locator verification, actual Git mutation-scope oracle, native control-plane authority composition, first native seed completion/consumption as applicable, and production-shaped composed lifecycles. Phase B begins only after `2.7.1` is remotely active, and it must not claim self-authorization because Phase A does not complete native repair authority or reuse bridge-004.

The Phase-B order is: future bridge-005 lifecycle; Tasks 5–9; `2.7.2` release preparation; full final verification; independent review; exact-SHA acceptance; publication; activation. `framework-contract-repair-bridge-005` is a future, separately reviewed and USER_APPROVER-approved migration authority. Its payload and scope must be prepared later, after `2.7.1` is remotely active; this Plan neither creates bridge-005 nor authorizes its creation. `STATE-ORACLE-SCHEMA-REQUIRED-FIELDS-001`, Plugin work, `GLOBAL_OPTIMUM_OVER_LOCAL_OPTIMUM`, and unrelated Framework refactors remain deferred.
