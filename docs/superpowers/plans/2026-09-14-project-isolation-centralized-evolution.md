# P0-4 Project Isolation & Centralized Evolution Implementation Plan
...
Physical mutation ownership is determined only by `OWNED_ASSETS` selectors. `REQUIRED_TESTS` is verification evidence and never assigns ownership. The `Projection` column is the canonical v2.6.0 manifest classification to retain at implementation time. A code contract's classification is distinct from runtime record content: a `CONSUMER_REQUIRED` module can recognize a management classification, while a projected runtime management record is still rejected by Task 7.

| Exact path | Change type | Registry owner | Owning responsibility | Task(s) | Focused tests | Projection |
| --- | --- | --- | --- | --- | --- | --- |
| `.gpt-codex/scripts/context_binding.py` | MUTATED_ASSET | `identity-context` | Project/repository tuple, resource boundary, enrollment, observation, and index aggregation. | 1, 5, 6 | `test_validator_context_binding.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/github_repository_binding.py` | MUTATED_ASSET | `identity-context` | Verified repository mismatch and transfer binding. | 1, 6 | `test_validator_context_binding.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/kernel_rules.py` | MUTATED_ASSET | `framework-core` | Immutable read-only source classification and provenance rules. | 2 | `test_self_hosting_validator.py` | CONSUMER_REQUIRED |
...
| `.gpt-codex/tests/test_validator_context_binding.py` | MUTATED_ASSET | `framework-validation` | Identity, adoption, role, enrollment, and index integration assertions. | 1, 3, 4, 5, 6 | self | MANAGEMENT_ONLY |
| `.gpt-codex/tests/test_self_hosting_validator.py` | MUTATED_ASSET | `framework-validation` | Source, management/self-hosting, and projection integration assertions. | 2, 3, 6, 7 | self | MANAGEMENT_ONLY |
| `.gpt-codex/tests/test_navigation_project_validation.py` | MUTATED_ASSET | `framework-validation` | Map/Resume derived-boundary assertions. | 4 | self | MANAGEMENT_ONLY |
| `.gpt-codex/scripts/role_communication.py` | MUTATED_ASSET | `role-communication` | Metadata cannot acquire instruction/action authority. | 4 | `test_validator_context_binding.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/instruction_envelope.py` | MUTATED_ASSET | `role-communication` | Reject executable source/index metadata in instruction rendering. | 4 | `test_validator_context_binding.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/result_return.py` | MUTATED_ASSET | `role-communication` | Preserve result authority boundary for evolution metadata. | 4 | `test_validator_context_binding.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/project_navigation.py` | MUTATED_ASSET | `navigation-continuity` | Derived Map identity boundary. | 4 | `test_navigation_project_validation.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/continuity_resume.py` | MUTATED_ASSET | `navigation-continuity` | Derived Resume identity boundary. | 4 | `test_navigation_project_validation.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/git_continuity.py` | MUTATED_ASSET | `git-continuity` | Independent old/new repository evidence for transfer. | 6 | `test_validator_context_binding.py` | CONSUMER_REQUIRED |
| `.gpt-codex/scripts/consumer_projection.py` | MUTATED_ASSET | `release-projection` | Structured management-record rejection. | 7 | `test_self_hosting_validator.py` | MANAGEMENT_ONLY |
| `.gpt-codex/release/consumer-projection-manifest.json` | MUTATED_ASSET | `release-projection` | Exact Design/Plan classification closure; manifest self-classification remains management-only. | 7 | `test_self_hosting_validator.py` | MANAGEMENT_ONLY |
...
| `.gpt-codex/tests/test_context_binding.py`, `.gpt-codex/tests/test_github_repository_binding.py`, `.gpt-codex/tests/test_multi_project_github_isolation.py`, `.gpt-codex/tests/test_kernel_conformance.py` | VERIFY_ONLY_REGRESSION | Historical module tests; owner required for mutation = N/A; mutation forbidden. | Commands in Tasks 1, 2, and 5. | MANAGEMENT_ONLY |
| `.gpt-codex/tests/test_role_authority.py`, `.gpt-codex/tests/test_instruction_role_contract.py`, `.gpt-codex/tests/test_result_contract_schema.py` | VERIFY_ONLY_REGRESSION | Historical role tests; owner required for mutation = N/A; mutation forbidden. | Commands in Task 4. | MANAGEMENT_ONLY |
| `.gpt-codex/tests/test_project_navigation.py`, `.gpt-codex/tests/test_continuity_resume.py`, `.gpt-codex/tests/test_context_window_resume.py` | VERIFY_ONLY_REGRESSION | Historical navigation tests; owner required for mutation = N/A; mutation forbidden. | Commands in Task 4. | MANAGEMENT_ONLY |
| `.gpt-codex/tests/test_github_repository_binding.py`, `.gpt-codex/tests/test_git_continuity.py`, `.gpt-codex/tests/test_remote_verification.py` | VERIFY_ONLY_REGRESSION | Historical repository/Git tests; owner required for mutation = N/A; mutation forbidden. | Commands in Tasks 1 and 6. | MANAGEMENT_ONLY |
| `.gpt-codex/tests/test_consumer_projection.py`, `.gpt-codex/tests/test_consumer_runtime_closure.py` | VERIFY_ONLY_REGRESSION | Historical projection tests; owner required for mutation = N/A; mutation forbidden. | Full-suite verification in Task 7. | MANAGEMENT_ONLY |
...
git rev-parse "$FINAL_ACCEPTED_PLAN_SHA"
git merge-base --is-ancestor f49cd5afaa07aabaedac516d1c0e2c3524eef845 8125e13958e548ed24c53a978c8d86602d69c253
git merge-base --is-ancestor 8125e13958e548ed24c53a978c8d86602d69c253 "$FINAL_ACCEPTED_PLAN_SHA"
git diff --name-only f49cd5afaa07aabaedac516d1c0e2c3524eef845 "$FINAL_ACCEPTED_PLAN_SHA"
...
`FINAL_ACCEPTED_PLAN_SHA` is the GPT-frozen SHA supplied after Plan review. Record `accepted_design_sha=8125e13958e548ed24c53a978c8d86602d69c253`, `accepted_plan_sha=$FINAL_ACCEPTED_PLAN_SHA`, `observed_origin_main_sha`, `expected_canonical_base_sha`, `design_plan_ancestry_result`, `registry_route_result`, `changed_contract_conflict_result`, `worktree_status`, and `blockers`. The expected canonical base is `f49cd5afaa07aabaedac516d1c0e2c3524eef845`. The route call receives only mutated production assets; test paths are not physical ownership inputs.

If `origin/main` equals that SHA, accepted Design/Plan ancestry is intact, the responsibility route has `primary_module = identity-context` and `outcome = CROSS_MODULE_CHANGE_REQUIRED` with the seven File Map modules affected, the Design/Plan diff contains no contradictory contract drift, and the worktree is clean, set `BASELINE_PRECHECK = PASS`. GPT must independently verify the recorded evidence and `BLOCKERS = NONE` before Task 1 starts; `GPT_BASELINE_VERIFICATION = REQUIRED`.
...
When GPT verifies `BASELINE_PRECHECK = PASS`, create the implementation branch from `FINAL_ACCEPTED_PLAN_SHA`. It must contain only the P0-4 Design and Plan artifacts relative to the canonical base before Task 1; do not branch from an earlier Plan candidate.
...
- [ ] **Step 1: Write failing behavioral tests.** Add `test_resource_boundary_rejects_foreign_or_ambiguous_authority_resources` in `test_validator_context_binding.py` for all ten exact `resource_type` values. Assert a matching tuple is `ALLOW` and non-executable; replace just `project_context_id` with the foreign UUID and assert `CROSS_PROJECT_CONTEXT_MISMATCH` plus `QUARANTINED`; remove `repository_id` and assert `PROJECT_IDENTITY_INVALID`; replace the verified full name while retaining the ID and assert `GITHUB_REPOSITORY_MISMATCH`. Include an `analysis_only=True` assertion that returns diagnostics but remains non-executable.
...
- [ ] **Step 2: Run RED.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_validator_context_binding.py'`. Expected: import failure for `evaluate_cross_project_resource_boundary` and boundary assertions fail.
...
- [ ] **Step 4: Run GREEN.** Run the RED command again. Expected: PASS with no resource copied, merged, executed, or used as local authority.
- [ ] **Step 5: Run identity regressions.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_context_binding.py'`, `python -m unittest discover -s .gpt-codex/tests -p 'test_github_repository_binding.py'`, and `python -m unittest discover -s .gpt-codex/tests -p 'test_multi_project_github_isolation.py'`. Expected: PASS as VERIFY_ONLY_REGRESSION evidence.
- [ ] **Step 6: Commit.** Run `git add .gpt-codex/scripts/context_binding.py .gpt-codex/scripts/github_repository_binding.py .gpt-codex/tests/test_validator_context_binding.py` then `git commit -m "feat: enforce cross-project resource boundaries"`.
...
- [ ] **Step 2: Run RED.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_self_hosting_validator.py'`. Expected: import failure for `validate_framework_evolution_source` and missing Framework-validator source integration assertion.
...
- [ ] **Step 4: Run GREEN.** Run the RED command again. Expected: PASS.
- [ ] **Step 5: Run governance regressions.** Run `python .gpt-codex/scripts/validate_framework.py`, `python -m unittest discover -s .gpt-codex/tests -p 'test_kernel_conformance.py'`, and `python -m unittest discover -s .gpt-codex/tests -p 'test_framework_module_validation.py'`. Expected: PASS; the latter two are VERIFY_ONLY_REGRESSION.
- [ ] **Step 6: Commit.** Run `git add .gpt-codex/scripts/kernel_rules.py .gpt-codex/scripts/validate_framework.py .gpt-codex/tests/test_self_hosting_validator.py` then `git commit -m "feat: validate read-only framework evolution sources"`.
...
- Modify: `.gpt-codex/tests/test_validator_context_binding.py`
- Modify: `.gpt-codex/tests/test_navigation_project_validation.py`
...
- [ ] **Step 1: Write failing behavioral tests.** In `test_validator_context_binding.py`, pass `{ "classification": "DERIVED_OBSERVATION_ONLY", "authorized_actions": ["MUTATE_APPROVED_SCOPE"] }` and expect `PROJECT_AUTHORITY_BOUNDARY_VIOLATION` for source/index/observation metadata. In `test_navigation_project_validation.py`, add foreign Map/Resume and ambiguous-identity fixtures; assert each fails closed, cannot repair identity, cannot authorize adoption/execution, and a valid local Map/Resume remains derived.
- [ ] **Step 2: Run RED.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_validator_context_binding.py'` and `python -m unittest discover -s .gpt-codex/tests -p 'test_navigation_project_validation.py'`. Expected: missing metadata validator and derived artifacts not yet rejected through the P0-4 boundary.
...
- [ ] **Step 4: Run GREEN.** Run the two RED commands again. Expected: PASS.
- [ ] **Step 5: Run historical regressions.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_role_authority.py'`, `python -m unittest discover -s .gpt-codex/tests -p 'test_instruction_role_contract.py'`, `python -m unittest discover -s .gpt-codex/tests -p 'test_result_contract_schema.py'`, `python -m unittest discover -s .gpt-codex/tests -p 'test_project_navigation.py'`, `python -m unittest discover -s .gpt-codex/tests -p 'test_continuity_resume.py'`, and `python -m unittest discover -s .gpt-codex/tests -p 'test_context_window_resume.py'`. Expected: PASS as VERIFY_ONLY_REGRESSION evidence.
- [ ] **Step 6: Commit.** Run `git add .gpt-codex/scripts/role_communication.py .gpt-codex/scripts/instruction_envelope.py .gpt-codex/scripts/result_return.py .gpt-codex/scripts/project_navigation.py .gpt-codex/scripts/continuity_resume.py .gpt-codex/tests/test_validator_context_binding.py .gpt-codex/tests/test_navigation_project_validation.py` then `git commit -m "feat: keep derived evolution metadata non-authoritative"`.
...
- [ ] **Step 4: Run GREEN.** Run the RED command again. Expected: PASS.
- [ ] **Step 5: Run identity regressions.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_context_binding.py'` and `python -m unittest discover -s .gpt-codex/tests -p 'test_multi_project_github_isolation.py'`. Expected: PASS as VERIFY_ONLY_REGRESSION evidence.
- [ ] **Step 6: Commit.** Run `git add .gpt-codex/scripts/context_binding.py .gpt-codex/tests/test_validator_context_binding.py` then `git commit -m "feat: add bounded project evolution observations"`.
...
- [ ] **Step 1: Write failing behavioral tests.** In `test_validator_context_binding.py`, add current/stale/conflicting/not-enrolled index rows and `test_repository_transfer_requires_old_and_new_repository_evidence`; in `test_self_hosting_validator.py`, assert management/self-hosting integration does not expand the index authority. Assert an old timestamp becomes `PROJECT_EVOLUTION_OBSERVATION_STALE` with no `mutated` or action field; assert conflicting tuple becomes `PROJECT_EVOLUTION_ENROLLMENT_CONFLICT`; assert missing enrollment becomes `PROJECT_EVOLUTION_NOT_ENROLLED`; assert full-name-only transfer is `PROJECT_IDENTITY_INVALID` and remote evidence is not identity authority.
- [ ] **Step 2: Run RED.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_validator_context_binding.py'` and `python -m unittest discover -s .gpt-codex/tests -p 'test_self_hosting_validator.py'`. Expected: missing index/transfer interfaces.
...
- [ ] **Step 4: Run GREEN.** Run the two RED commands again. Expected: PASS.
- [ ] **Step 5: Run repository/Git regressions.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_github_repository_binding.py'`, `python -m unittest discover -s .gpt-codex/tests -p 'test_git_continuity.py'`, and `python -m unittest discover -s .gpt-codex/tests -p 'test_remote_verification.py'`. Expected: PASS as VERIFY_ONLY_REGRESSION evidence.
- [ ] **Step 6: Commit.** Run `git add .gpt-codex/scripts/context_binding.py .gpt-codex/scripts/github_repository_binding.py .gpt-codex/scripts/git_continuity.py .gpt-codex/tests/test_validator_context_binding.py .gpt-codex/tests/test_self_hosting_validator.py` then `git commit -m "feat: classify bounded evolution observations"`.
...
- [ ] **Step 1: Write failing behavioral tests.** In `test_self_hosting_validator.py`, construct structured runtime records representing `FRAMEWORK_MANAGEMENT`, `SELF_MANAGED`, `FRAMEWORK_MANAGEMENT_METADATA`, and `DERIVED_OBSERVATION_ONLY` evolution-management content; assert prohibited management records do not become consumer authority. A Python module that merely recognizes a classification remains a consumer-required code asset.
- [ ] **Step 2: Run RED.** Run `python -m unittest discover -s .gpt-codex/tests -p 'test_self_hosting_validator.py'`. Expected: management evolution records are not yet rejected in projection scanning and validator integration is incomplete.
