...
**Files:** Create `feature/framework-design-continuity-sufficiency-implementation` from canonical main; modify no accepted Design/Plan artifact directly. **Inputs:** `accepted_design_sha=ec74a57d10cd7db1b2e56e122d4bbaf12b6a3ceb`, runtime `$ACCEPTED_PLAN_SHA`, and `canonical_main_sha=f49cd5afaa07aabaedac516d1c0e2c3524eef845`. After this Plan is Reviewer-accepted and frozen, the future GPT_ORCHESTRATOR `EXECUTION_INSTRUCTION` must explicitly supply `$ACCEPTED_PLAN_SHA` as exactly one immutable 40-hex commit SHA for the accepted/frozen P0-5 Plan. It is never inferred from branch HEAD, tag, conversation memory, Resume, or local state; branch/tag/moving refs may verify remote reachability only and are never merge operands or authority. Before any Task-0 repository mutation, missing/malformed SHA, unavailable commit object, SHA not remotely reachable, or absent Plan path at `$ACCEPTED_PLAN_SHA` returns `RECONCILIATION_REQUIRED` and prohibits merge.

**Evidence interface:** Return GPT-verifiable command evidence, not a new P0-5 persistence artifact, with exactly `accepted_design_sha`, `accepted_plan_sha`, `canonical_main_sha`, `implementation_base_sha`, `design_content_preserved`, `plan_content_preserved`, `registry_compatibility_result`, `state_schema_compatibility_result`, `navigation_api_compatibility_result`, `validation_api_compatibility_result`, `projection_compatibility_result`, `changed_path_conflicts`, `full_suite_result`, `framework_validation_result`, `project_validation_result`, and `blockers`. `accepted_plan_sha` echoes the exact immutable SHA supplied by the implementation instruction; `plan_content_preserved` compares the Plan blob in R against `$ACCEPTED_PLAN_SHA:docs/superpowers/plans/2026-09-14-framework-design-continuity-sufficiency.md`. `full_suite_result` is evidence produced after B0 projection closure, not an R acceptance gate. Prove compatibility by `git diff --name-status` against each accepted SHA, Registry routing on every intended path, schema/API comparison, fresh remote `ls-remote`, framework/project validators, projection validation, and—after B0—full tests.

- [ ] **Step 1: Merge accepted artifacts** — Validate that `$ACCEPTED_PLAN_SHA` was populated by the authoritative GPT implementation instruction as the immutable accepted Plan commit, is exactly 40 hex characters, resolves to an available commit object remotely, and contains the Plan path; otherwise stop `RECONCILIATION_REQUIRED`. Verify fresh `origin/main`, create the named branch/worktree at canonical main, then run `git merge --no-ff "$ACCEPTED_PLAN_SHA" -m "chore: reconcile P0-5 continuity plan with v2.6.0"`. Do not substitute `feature/framework-design-continuity-sufficiency-design`, `origin/feature/framework-design-continuity-sufficiency-design`, `HEAD`, `latest`, or a tag for this SHA. The reconciliation merge `R` must have parent 1 canonical main and parent 2 accepted Plan; no rebase, cherry-pick, amend, or history rewrite.
- [ ] **Step 2: Verify R tree and debt** — Prove `main → R` changes exactly the accepted Design and Plan paths, compare both blobs to accepted SHAs, and require framework/project validation plus schema/API/Registry compatibility to pass. Audit projection: unknown paths must be exactly the accepted P0-5 Design and Plan, missing required must be 0, and invalid classification must be 0. The repository-wide full suite is intentionally deferred until B0 because current-source projection tests correctly fail while these two accepted Plan-stage paths remain unclassified. Any other projection debt, path, or validation failure returns `RECONCILIATION_REQUIRED`.
- [ ] **Step 3: Create B0 and verify** — Modify only `.gpt-codex/release/consumer-projection-manifest.json`, classify both accepted paths `DEVELOPMENT_HISTORY`, commit one normal `R → B0`, then require projection unknown = 0, projection missing required = 0, projection invalid classification = 0, framework/project validation PASS, full unittest suite = 0 failures / 0 errors, and a clean worktree.
- [ ] **Step 4: Hard stop** — **NO TASK 1 OR LATER MAY EXECUTE UNTIL GPT independently verifies R parents, main→R paths/blobs, R's exact two-path projection debt, R→B0 manifest-only scope, B0 projection closure, B0 full-suite zero-failure evidence, clean worktree, and P0_5_CLEAN_IMPLEMENTATION_BASELINE = GPT_VERIFIED with BLOCKERS = NONE.**
