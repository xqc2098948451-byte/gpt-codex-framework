...
1. **Approval creation:** `GPT_ORCHESTRATOR` issues an `APPROVAL_REQUEST` bound to the proposed reconciliation instruction. `USER_APPROVER` creates one production-shaped `APPROVAL_RESULT`; creation is a response, not a control-plane admission.
2. **Approval durability:** `USER_APPROVER` alone transports that exact result through the existing Git-continuity remote into a protected, fast-forward-only evidence ref, `refs/heads/gpt-codex-approval-evidence`. The result is stored as `approvals/<APPROVAL_RESULT.result_id>.json` in an evidence commit. The immutable authority locator is the closed tuple `{remote_ref, evidence_commit_sha, path, blob_sha}`: the gate resolves `evidence_commit_sha:path`, requires the resulting blob SHA to equal `blob_sha`, and validates the resulting JSON as the approval result. The mutable ref is only a reachability/transport check; its current head is never authority. A replaced or advanced ref cannot substitute a different commit/blob for the bound tuple. `CODEX_IMPLEMENTER` cannot create, push, rewrite, or select approval evidence.
...
The repair adds `APPROVAL_RESULT` rather than overloading a review result. It is mandatory in `.gpt-codex/schemas/result-envelope.schema.json`, `.gpt-codex/scripts/role_communication.py`, the result-envelope/taxonomy validation path, existing Git-continuity verification, any result/template surface, focused result-taxonomy/schema tests, and composed control-plane lifecycle tests. The schema must close the `approved_instruction` object and require `instruction_id`, `expected_state_revision`, `expected_base_sha`, normalized `scope_paths`, project/repository identity, and the exact target Work Unit reference. It must also close and require `durable_evidence` as the `{remote_ref, evidence_commit_sha, path, blob_sha}` tuple above. `role_communication.py` must register `APPROVAL_RESULT` as a result message type and enforce that only `USER_APPROVER` produces it.

The exact production shape is: `result_message_type = APPROVAL_RESULT`; `responder_role = USER_APPROVER`; `status = PASS`; `decision = APPROVE | REJECT`; `response_to_instruction_id = APPROVAL_REQUEST.instruction_id`; `approved_instruction` is the closed object above; `evidence_refs = ["git:<evidence_commit_sha>:<path>:<blob_sha>"]`; `completion_gate = NONE`; `remote_verification = VERIFIED`; and `completion_evidence = null` (not applicable to an approval transaction). `PASS` means that the approval response was correctly formed and durably transported, not that its decision approves the work; only `decision` distinguishes `APPROVE` from `REJECT`. The Result schema must retain the remote-verification requirement for this PASS result but explicitly exclude `APPROVAL_RESULT` from execution-style `completion_evidence` requirements.

The correlation chain is exact: `APPROVAL_REQUEST.in_response_to_instruction_id = RECONCILIATION_REQUEST.instruction_id`; `APPROVAL_RESULT.response_to_instruction_id = APPROVAL_REQUEST.instruction_id`; and `APPROVAL_RESULT.approved_instruction.instruction_id = RECONCILIATION_REQUEST.instruction_id`. The gate verifies all three before it reads the immutable evidence tuple.
...
It may not touch `STATE`, any pre-existing Work Unit, `CONTROL`, product files, Plugin files, release artifacts, or any schema other than the named Instruction, Result, and Work Unit schemas. It creates no general bootstrap subsystem and performs no schema self-authorization beyond this exact repair candidate.
...
After `USER_ACCEPTANCE`, a separately authorized integration/publication action must fast-forward `main` to the exact independently reviewed candidate SHA; merge commits, a different SHA, and force push fail. A Framework patch release/activation is required because this changes Framework authority: it must publish the Framework version associated with that exact fast-forwarded main SHA and update the existing Framework adoption/activation contract (`CONTROL.framework.adopted_version` and its remote-continuity verification) to that release. The repaired rules become active only after `REMOTE_ACTIVE_AUTHORITY_VERIFICATION` proves the configured active remote ref is `refs/heads/main`, its head is the exact reviewed SHA, and the activated Framework version resolves to that same SHA. The bridge terminates only at that point, not when the candidate merely exists on a review branch.
...
- **How are post-bridge approvals made durable without STATE mutation?** `USER_APPROVER` transports the result through the protected Git-continuity evidence ref. The gate consumes its exact evidence commit/blob tuple, never the mutable ref head; `STATE.evidence_refs` is only a later admitted projection.
- **What makes an approval result a PASS without confusing approval with execution?** `status = PASS` means the response/transport succeeded, while `decision` carries APPROVE/REJECT. `completion_evidence = null` is explicitly inapplicable, though remote verification remains required.
