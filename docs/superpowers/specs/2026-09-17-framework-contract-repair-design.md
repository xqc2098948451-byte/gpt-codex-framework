...
4. The clean `a112efcc24c969369224804a6fc1e5961626c05b` baseline cannot satisfy Contract Repair release or final verification because consumer projection rejects durable management Work Units that the current manifest leaves unclassified. This pre-existing release-closure prerequisite was discovered only after the bridge-authorized implementation worktree clean-baseline check; it is a real baseline projection regression, distinct from the contract defects that are the original Contract Repair purpose.
...
- Materialize the exact three-path Baseline closure target as a release-closure prerequisite so the Contract Repair candidate can execute full-suite, consumer-projection, packaging, and final verification: the manifest and local-corrective test remain byte-identical to the preserved Baseline source, while the consumer-projection test uses the explicitly reconciled, content-locked candidate bytes defined in section 14.2.

## 4. Non-goals

This amendment permits only the exact three-path Baseline closure target defined in section 14.2 to enter the Contract Repair candidate as a release-closure prerequisite. The preserved Baseline source worktree remains read-only and unchanged: the manifest and local-corrective test are reproduced byte-identically, while the consumer-projection test is replaced only by the explicitly reconciled, content-locked candidate bytes. This does not implement `STATE-ORACLE-SCHEMA-REQUIRED-FIELDS-001`, close the Baseline Work Unit lifecycle, execute the post-activation native Baseline checkpoint early, resume Task 3 Plugin, import any other Baseline worktree change, redesign consumer projection production behavior, change Framework module count, add admin/root authority, a database, registry, queue, new control-plane subsystem, or a general role-system redesign. The State Oracle required-fields repair remains deferred until repaired native authority is active. This Design defines the required later integration/publication/activation boundary for the repair but does not authorize performing it.
...
The replacement bridge has exactly 38 exact repository-relative authorized paths, including only `.gpt-codex/release/consumer-projection-manifest.json`, `.gpt-codex/tests/test_consumer_projection.py`, and `.gpt-codex/tests/test_local_corrective_state.py` as the Baseline closure addition. The existing three `DELETE_ONLY` v2.7.0 distribution paths remain unchanged; no 39th path is authorized. The earlier 35-path signed bridge remains historical and `superseded-before-consumption`: `OLD_BRIDGE_CONSUMPTION = NO`; `OLD_BRIDGE_REPLAY = FORBIDDEN`. The subsequently approved 38-path `framework-contract-repair-bridge-002` reached partial local `USER_LOCAL` execution for Tasks 1–3 but produced no repair candidate commit, push, publication, or activation. It is superseded by this reconciliation and retained as immutable predecessor/audit evidence only; its partial local bytes are not completion evidence for a successor bridge and are not replayable. Any successor replacement bridge must be independently reviewed and explicitly `USER_APPROVER`-approved; it cannot be inferred from either predecessor bridge.

`PRESERVED_BASELINE_SOURCE` remains read-only and byte-unchanged. Its three historical SHA-256 values are: `.gpt-codex/release/consumer-projection-manifest.json` = `A7CAEC033DAA058877C7C24DD745FF3DCD36B51335C84CD4553C69ABA2DCC195`; `.gpt-codex/tests/test_consumer_projection.py` = `E51159943EAC482F39F1C6A23665943AE22AB3228B4EF45BA990A388FDF240CC`; `.gpt-codex/tests/test_local_corrective_state.py` = `AA5DBB9346E7DBBE47A27D4E300403485029CD00B40CE7A68E49387DA626AE98`. `BASELINE_CLOSURE_TARGET` remains exactly those same three repository paths, but its content-locked target hashes are: `.gpt-codex/release/consumer-projection-manifest.json` = `A7CAEC033DAA058877C7C24DD745FF3DCD36B51335C84CD4553C69ABA2DCC195`; `.gpt-codex/tests/test_consumer_projection.py` = `BD69FF61E50ED4E3818A2F61BA49C6AC9790192D8E94B1DCCA747F50AF34A2DF`; `.gpt-codex/tests/test_local_corrective_state.py` = `AA5DBB9346E7DBBE47A27D4E300403485029CD00B40CE7A68E49387DA626AE98`. The manifest and local-corrective target bytes are reproduced byte-identically from the preserved source. The consumer-projection target bytes are the explicitly reconciled UTF-8-without-BOM/LF candidate whose sensitivity oracle derives the exact durable `.gpt-codex/work-units/*.json` set from the candidate snapshot: normal projection classifies every such Work Unit `MANAGEMENT_ONLY` and excludes it from consumer inventory; removing `.gpt-codex/work-units/` makes the unknown Work Unit paths equal that exact durable set; restoring the prefix returns PASS. No historical Work Unit count is authority. No fourth Baseline closure path is authorized. The target's only role is a release-closure prerequisite, not new design authority, a new Baseline implementation effort, consumer-projection production redesign, or scope for opportunistic cleanup. Implementation must reproduce exactly these target bytes; any target-byte difference requires reconciliation rather than ad hoc correction.

The replacement bridge can touch only its exact 38-path immutable list; the following repair-surface description and focused test/template categories are explanatory and cannot expand that list:
...
The replacement bridge lifecycle has these exact pre-repair phases: `REPLACEMENT_BRIDGE_AUTHORIZED -> BASELINE_CLOSURE_SNAPSHOT_APPLY -> BASELINE_CLOSURE_FOCUSED_VERIFY -> CLEAN_BASELINE_FULL_VERIFY -> CONTRACT_REPAIR_TASK_1`, then `LOCAL_REPAIR -> REMOTE_REVIEW_CANDIDATE -> INDEPENDENT_POST_REPAIR_REVIEW -> USER_ACCEPTANCE -> INTEGRATION/PUBLICATION -> FRAMEWORK_ACTIVATION -> REMOTE_ACTIVE_AUTHORITY_VERIFICATION -> BRIDGE_TERMINATED`. Baseline closure verification proves consumer-projection focused tests PASS, `validate_consumer_projection.py --root .` PASS, full `unittest discover` completion with known counts and zero failures/errors, `validate_framework.py` PASS, `validate_project.py .` PASS, and `git diff --check` PASS; it does not require Baseline Work Unit lifecycle closure at this point. A review-branch commit is only a candidate and never active authority. While it awaits review or acceptance, the replacement bridge remains auditable but is mutation-disabled: it cannot authorize another edit, commit, rebase, scope expansion, control-plane transition, or Work Unit.
...
This chooses activation shape B. The final 2.7.1 repair candidate remains one integrated candidate: content-locked Baseline closure target bytes + Contract Repair Tasks 1–10 + 2.7.1 release surfaces. It does not authorize a separate Baseline commit before the single final repair candidate commit; the replacement bridge permits `repair_candidate_commit_limit = 1`. Before final independent post-repair review, that candidate must include the exact release-version and generated activation surfaces listed in section 14.2, and the review must bind its final commit SHA. After `USER_ACCEPTANCE`, a separately authorized integration/publication action must fast-forward `main` to that exact independently reviewed SHA; merge commits, a different SHA, and force push fail. It then creates immutable tag `v<release-version>` at that same main SHA and publishes the already-reviewed canonical release artifact through the existing release contract, without another tracked-file commit. `FRAMEWORK_ACTIVE_SHA = the tag target SHA, independently verified to equal refs/heads/main at the reviewed SHA.` A tracked activation mutation after final review is forbidden; it requires a new candidate and independent review. The bridge terminates only after `REMOTE_ACTIVE_AUTHORITY_VERIFICATION` proves this equality, not when the candidate merely exists on a review branch.
...
The bridge creates `framework-baseline-checkpoint-control-plane-001` only for the first post-activation native transition. After `REMOTE_ACTIVE_AUTHORITY_VERIFICATION = PASS`, the Framework still performs this revision-12 native Baseline checkpoint and returns to the Baseline Work Unit for fresh `REVIEW_FINDING -> fresh adjudication -> State Oracle required-fields FIX -> Baseline lifecycle closure`. Earlier incorporation of the content-locked three-path Baseline closure target does not pretend these later governance transitions already happened. The seed has `basis_state_revision = 12`, state `AUTHORIZED`, and `scope.owned_paths` exactly equal to the frozen four-path Baseline checkpoint: `.gpt-codex/STATE.json`, `.gpt-codex/work-units/framework-baseline-stabilization-001.json`, `.gpt-codex/evidence/results/RESULT-BASELINE-STABILIZATION-POSTEXEC-FINDING.json`, and `.gpt-codex/evidence/results/RESULT-FIX-REMEDIATION-LIFECYCLE-CONTRACT-RECONCILIATION.json`. It authorizes no Framework implementation/schema/script/test path, no Plugin path, no `CONTROL` path, and does not contain `.gpt-codex/work-units/framework-baseline-checkpoint-control-plane-001.json` in its scope.
...
Deferred: the STATE required-fields repair; every Baseline worktree change other than the exact three-path `BASELINE_CLOSURE_TARGET`; Task 3 Plugin; frozen revision-13 checkpoint; all implementation; and any Plan until this Design receives GPT review and user approval. `GLOBAL_OPTIMUM_OVER_LOCAL_OPTIMUM` remains separate post-Contract-Repair Framework optimization work and is not Kernel or Design authority here. A separately authorized integration/publication/activation action remains required after post-repair review and user acceptance; this Design does not authorize it.
...
- **Can the Baseline closure target bypass later Baseline governance?** No: it is only the content-locked three-path release-closure prerequisite; after remote activation the existing revision-12 checkpoint, fresh review/adjudication, State Oracle required-fields FIX, and Baseline lifecycle closure still occur under repaired native authority.
- **Can the historical 35-path bridge be reused?** No: it was superseded before consumption and replay is forbidden.
...
`framework-contract-repair-bridge-003` retains its exact immutable old Design/Plan binding. This amendment does not rewrite, reinterpret, or expand it. It may continue only for operations already authorized by that old authority: because Prelude and Tasks 1–4 have unchanged technical semantics, it may consume only their overlapping portion. After Tasks 1–4 verification, execution must stop; bridge-003 does not authorize the new checkpoint-release ordering, Phase-A final verification, `2.7.1` release-candidate preparation under this amended Design/Plan, publication, or Tasks 5–9.

After the corrected phase-split Design and Plan are accepted and Prelude plus Tasks 1–4 pass, a future separately prepared `framework-contract-repair-bridge-004` is required for Phase A. It must be independently PRE_EXECUTION reviewed, exact-payload USER_APPROVER approved, remotely verified before use, bound to the corrected Design/Plan exact SHAs, limited to the minimum exact scope for Phase-A final verification, `2.7.1` release surfaces, one reviewed candidate-commit/publication lifecycle, and remote verification, and non-reusable. When bridge-004 is remotely authorized, bridge-003 is superseded and terminated for further mutation. This Design does not create bridge-004.

The Phase-A order is therefore: bridge-003 immutable old authority; Prelude; Tasks 1–4; verify Tasks 1–4; STOP bridge-003 execution; bridge-004 preparation/review/approval/remote verification; Phase-A final verification; `2.7.1` release candidate; independent `POST_EXECUTION` review; exact-SHA `USER_APPROVER` acceptance; governed publication; remote `2.7.1` verification; terminate bridge-004; then pause feature development. Before publication, the exact candidate must pass Prelude, Tasks 1–4, the full test suite, `validate_framework.py`, `validate_project.py`, `validate_consumer_projection.py`, `git diff --check`, authorized changed-path verification, and independent `POST_EXECUTION` review. USER_LOCAL may perform publication only after exact-SHA acceptance.

Phase B begins only after `2.7.1` is remotely active. Former Tasks 5–9 retain their existing technical semantics but target `2.7.2`: external approval-evidence locator verification, actual Git mutation-scope oracle, native control-plane authority composition, first native seed completion/consumption as applicable, and production-shaped composed lifecycles. Because `2.7.1` does not contain complete native repair authority, Phase B cannot self-authorize or reuse bridge-004. A future separately reviewed and explicitly USER_APPROVER-approved `framework-contract-repair-bridge-005` is required; its exact payload and scope are prepared only after `2.7.1` is remotely active. This Design creates neither bridge-005 nor authority to create it.
