...
- Restore the already-authorized preserved Baseline three-file snapshot as a release-closure prerequisite so the Contract Repair candidate can actually execute full-suite, consumer-projection, packaging, and final verification.

## 4. Non-goals

This amendment permits only the already-frozen three-file Baseline snapshot to enter the Contract Repair candidate as a release-closure prerequisite. It does not implement `STATE-ORACLE-SCHEMA-REQUIRED-FIELDS-001`, close the Baseline Work Unit lifecycle, execute the post-activation native Baseline checkpoint early, resume Task 3 Plugin, import any other Baseline worktree change, redesign consumer projection, change Framework module count, add admin/root authority, a database, registry, queue, new control-plane subsystem, or a general role-system redesign. The State Oracle required-fields repair remains deferred until repaired native authority is active. This Design defines the required later integration/publication/activation boundary for the repair but does not authorize performing it.
...
The replacement bridge has exactly 38 exact repository-relative authorized paths, including only `.gpt-codex/release/consumer-projection-manifest.json`, `.gpt-codex/tests/test_consumer_projection.py`, and `.gpt-codex/tests/test_local_corrective_state.py` as the Baseline closure addition. The existing three `DELETE_ONLY` v2.7.0 distribution paths remain unchanged; no 39th path is authorized. `OLD_SIGNED_BRIDGE = historical / superseded-before-consumption`; `OLD_BRIDGE_CONSUMPTION = NO`; `OLD_BRIDGE_REPLAY = FORBIDDEN`. A replacement bridge must be independently reviewed and explicitly `USER_APPROVER`-approved; it cannot be inferred from the old bridge.

`BASELINE_CLOSURE_SNAPSHOT` is content-locked by these exact SHA-256 values: `.gpt-codex/release/consumer-projection-manifest.json` = `A7CAEC033DAA058877C7C24DD745FF3DCD36B51335C84CD4553C69ABA2DCC195`; `.gpt-codex/tests/test_consumer_projection.py` = `E51159943EAC482F39F1C6A23665943AE22AB3228B4EF45BA990A388FDF240CC`; `.gpt-codex/tests/test_local_corrective_state.py` = `AA5DBB9346E7DBBE47A27D4E300403485029CD00B40CE7A68E49387DA626AE98`. No fourth Baseline snapshot path is authorized. Its only role is a release-closure prerequisite, not new design authority, a new Baseline implementation effort, or scope for opportunistic cleanup. Implementation must reproduce exactly those bytes; any byte difference requires reconciliation rather than ad hoc correction.
...
- **Can the frozen Baseline closure snapshot bypass later Baseline governance?** No: it is only the content-locked three-file release-closure prerequisite; after remote activation the existing revision-12 checkpoint, fresh review/adjudication, State Oracle required-fields FIX, and Baseline lifecycle closure still occur under repaired native authority.
- **Can the historical bridge be reused?** No: it is superseded before consumption and replay is forbidden; the 38-path replacement bridge requires independent review and explicit `USER_APPROVER` approval.
