...
| Complete binding belongs to a different active Project context | `CROSS_PROJECT_CONTEXT_MISMATCH`; reject and quarantine the reference. | Prohibited. | Re-bind or reconcile from the owning Project with its local authorization and evidence. |
| Required strong identity is missing, partial, conflicting, foreign, or inferred | `PROJECT_IDENTITY_INVALID`; reject and quarantine the reference. | Prohibited. | Establish an explicit Project-local binding. |
| Verified repository binding contradicts the resource repository identity | `GITHUB_REPOSITORY_MISMATCH`; reject and quarantine the reference. | Prohibited. | Reconcile the verified repository binding locally. |
...
| `PROJECT_EVOLUTION_EVALUATION_CONTRACT` | Project-local evaluator | Project decision process; optional observation producer | source plus local authoritative context | existing local compatibility outcome | `FRAMEWORK_ADOPTION_NOT_AUTHORIZED`, `PROJECT_AUTHORITY_BOUNDARY_VIOLATION`, `PROJECT_IDENTITY_INVALID`, `CROSS_PROJECT_CONTEXT_MISMATCH`, `GITHUB_REPOSITORY_MISMATCH` | No. |
| `PROJECT_ADOPTION_DECISION_CONTRACT` | Project `CONTROL`/Work Unit/Role Protocol | Project-local migration executor | local evaluation and explicit decision | local authorized decision and Work Unit binding | `FRAMEWORK_ADOPTION_NOT_AUTHORIZED`, `PROJECT_AUTHORITY_BOUNDARY_VIOLATION`, `PROJECT_IDENTITY_INVALID`, `CROSS_PROJECT_CONTEXT_MISMATCH`, `GITHUB_REPOSITORY_MISMATCH` | Yes, only in owning Project root. |
...
| `GITHUB_REPOSITORY_MISMATCH` | Verified repository binding contradicts the resource repository identity. | Existing repository-binding contract. | None. | Local verified binding/reconciliation. |
| `PROJECT_IDENTITY_INVALID` | Required strong identifier is missing, partial, foreign, conflicting, or inferred. | Existing identity contract. | None. | Explicit Project-local binding. |
| `PROJECT_AUTHORITY_BOUNDARY_VIOLATION` | Framework, index, or foreign-Project metadata attempts to become Project authority. | Kernel/Role Protocol. | None. | Remove unauthorized action; use local authorization. |
| `FRAMEWORK_ADOPTION_NOT_AUTHORIZED` | Framework adoption or migration lacks explicit Project-local decision and Work Unit. | Project controls. | None. | Create authorized local decision and Work Unit. |
