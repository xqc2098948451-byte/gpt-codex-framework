...
`target_work_unit` remains the logical Work Unit ID. The existing Instruction Envelope gains one schema-level optional locator, `target_work_unit_ref = {"path": "<repository-relative path>", "sha": "<40-hex Git revision>"}`, because two remediation attempts proved that logical identity alone cannot bind a caller mapping to an immutable repository Work Unit. Its resolved object must have `work_unit_id == target_work_unit`; otherwise validation fails closed. The locator creates neither a Work Unit lookup subsystem nor a registry: root-aware validation resolves its path at its Git revision inside the current repository, then verifies the Work Unit project, authorization state, state revision, and immutable Design/Plan refs where required. Legacy Instructions remain parseable, while a governed repository-backed mutation without this locator is denied.

For a Strategy change, the old baseline is `CONTROL.execution_policy` at `Instruction.expected_base_sha` and the current accepted profile is current `CONTROL.execution_policy`. If they differ, the resolved `AUTHORIZED` Work Unit must own `.gpt-codex/CONTROL.json`, the Instruction must target that locator and bind the old base SHA, `MUTATE_APPROVED_SCOPE` must be authorized, and project/state/artifact correlation must pass. This existing Git + Work Unit + Instruction chain is the governed Strategy-change authority; no Strategy-change artifact, history database, or `requirement_ref` authority is added.
...
This Foundation creates no new schema file, Framework module, database, project-context store, conversation store, state machine, approval artifact, Execution Context identity subsystem, Agent Registry, Reviewer Registry, Session Manager, Context Server, automatic learning, automatic strategy switching, automatic scoring, private chain-of-thought storage, raw-prompt archive, scratchpad archive, token trace, Resource Governor, quota manager, token scheduler, parallelism optimizer, second review lifecycle, second Work Unit system, or second compatibility/update system. The only permitted existing-schema extension is the optional `target_work_unit_ref` in `instruction-envelope.schema.json`, justified by the proven repository-backed locator gap. It also creates no Plugin, `.agents/` directory, Plugin manifest, Plugin Skill, marketplace configuration, or Plugin updater.

Any implementation must reuse existing schema where it can express the required contract. If a required Design capability cannot be expressed by the existing structure, execution stops with an explicit proven blocker; it must not infer a new schema file, module, subsystem, or context system beyond the explicitly accepted Instruction locator extension.
